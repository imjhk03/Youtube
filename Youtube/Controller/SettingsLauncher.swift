...
class SettingsLauncher: NSObject, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    let settings: [Setting] = {
        let settingsSetting = Setting(name: .Settings, imageName: "cancel")
    ...
    @objc func handleDismiss(_ setting: NSObject) {
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: .curveEaseOut, animations: {
            ...
            if setting is Setting && (setting as! Setting).name != .Cancel {
                self.homeController?.showControllerForSetting(setting as! Setting)
            }
        }
    }
